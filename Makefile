...
release-podman-managed-pass-smoke: ## Block on rootless Podman validation of the exact host-architecture artifact image
	@set -eu; \
		...
		for role_identity in runtime:21001 control:21002 edge:21003 registry:21004; do \
			...
			mounts="-v $$tmp/roles/$$role:/private"; state_volume=; if [ "$$role" != edge ]; then state_volume="gordon-release-podman-$$role-$$$$-g1"; podman volume create "$$state_volume" >/dev/null; generation_volumes="$$generation_volumes $$state_volume"; mounts="$$mounts -v $$state_volume:/var/lib/gordon"; fi; if [ "$$role" = runtime ]; then mounts="$$mounts -v $$tmp/runtime.sock:/run/gordon/runtime.sock:ro"; fi; \
			podman run --detach --name "$$name" --user "$$identity:$$identity" --userns "keep-id:uid=$$identity,gid=$$identity" --group-add 21900 --cap-drop ALL --security-opt no-new-privileges $$mounts --entrypoint sh "$$image" -ec 'trap "exit 0" TERM INT; while :; do sleep 1; done' >/dev/null; \
			...
			test "$$(podman inspect --format '{{json .HostConfig.GroupAdd}}' "$$name")" = '["21900"]'; podman exec "$$name" id -G | tr ' ' '\n' | grep -qx 21900; \
			...
			if [ -n "$$state_volume" ]; then test "$$(podman inspect --format '{{range .Mounts}}{{if eq .Destination \"/var/lib/gordon\"}}{{.Name}}{{end}}{{end}}' "$$name")" = "$$state_volume"; podman exec "$$name" sh -ec 'probe=/var/lib/gordon/.identity-write-check; : >"$$probe"; test -r "$$probe"; rm "$$probe"; test ! -e "$$probe"'; fi; \
...
release-image-smoke: ## Verify artifact-derived amd64/arm64 images and a real monolith runtime
	@set -eu; \
		...
		for arch in amd64 arm64; do \
			...
			docker run --rm --platform "linux/$$arch" --user 0:0 --entrypoint sh "$$image" -ec 'test "$$(stat -c %u:%g:%a /var/lib/gordon)" = 0:21900:770; test "$$(stat -c %u:%g:%a /var/lib/gordon/secrets)" = 21002:21002:700'; \
			...
			mkfifo "$$lease_dir/ready"; docker run --rm --name "$$owner" --platform "linux/$$arch" --user 21002:21002 --group-add 21900 -v "$$secrets_volume:/var/lib/gordon/secrets" -v "$$secrets_config:/tmp/gordon.toml:ro" -e GNUPGHOME=/var/lib/gordon/secrets/current/gnupg -e PASSWORD_STORE_DIR=/var/lib/gordon/secrets/current/password-store "$$image" secrets lock --config /tmp/gordon.toml >"$$lease_dir/ready" & owner_pid=$$!; \
			...
			if docker run --rm --platform "linux/$$arch" --user 21002:21002 --group-add 21900 -v "$$secrets_volume:/var/lib/gordon/secrets" -v "$$secrets_config:/tmp/gordon.toml:ro" -e GNUPGHOME=/var/lib/gordon/secrets/current/gnupg -e PASSWORD_STORE_DIR=/var/lib/gordon/secrets/current/password-store "$$image" secrets doctor --config /tmp/gordon.toml >"$$lease_error" 2>&1; then exit 1; fi; \
			...
			for restart in 1 2; do docker run --rm --platform "linux/$$arch" --user 21002:21002 --group-add 21900 -v "$$secrets_volume:/var/lib/gordon/secrets" -v "$$secrets_config:/tmp/gordon.toml:ro" -e GNUPGHOME=/var/lib/gordon/secrets/current/gnupg -e PASSWORD_STORE_DIR=/var/lib/gordon/secrets/current/password-store "$$image" secrets doctor --config /tmp/gordon.toml --write-check >/dev/null; done; \
			docker run --rm --platform "linux/$$arch" --user 21002:21002 --group-add 21900 -v "$$secrets_volume:/var/lib/gordon/secrets:ro" --entrypoint sh "$$image" -ec 'test -s /var/lib/gordon/secrets/current/.gordon-managed-pass-fingerprint; test -s /var/lib/gordon/secrets/current/password-store/.gpg-id; test -d /var/lib/gordon/secrets/current/gnupg'; \
