...
release-podman-managed-pass-smoke: ## Block on rootless Podman validation of the exact host-architecture artifact image
	@set -eu; \
		...
		for role_identity in runtime:21001 control:21002 edge:21003 registry:21004; do \
			...
			mounts="-v $$tmp/roles/$$role:/private"; state_volume=; if [ "$$role" = runtime ] || [ "$$role" = control ]; then state_volume="gordon-release-podman-$$role-$$$$-g1"; podman volume create "$$state_volume" >/dev/null; generation_volumes="$$generation_volumes $$state_volume"; mounts="$$mounts -v $$state_volume:/var/lib/gordon"; fi; if [ "$$role" = runtime ]; then mounts="$$mounts -v $$tmp/runtime.sock:/run/gordon/runtime.sock:ro"; fi; \
