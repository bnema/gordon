...
Do not paste role environment files, tokens, Authorization headers, private host paths, raw compatibility artifacts, or secret-provider output. Preserve migration checkpoints/outboxes for diagnosis. A handoff must explicitly state whether public listener authority is monolith, prepared, or switched and whether runtime authority transferred. Use [architecture handoff](./architecture-handoff.md) for role-boundary changes and [TDD handoff](./tdd-handoff.md) for implementation evidence.
