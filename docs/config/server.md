...
| `max_blob_chunk_size` | string | `"512MB"` | Maximum request body size for a single registry blob upload chunk |
...
```toml
[server]
max_blob_chunk_size = "512MB"  # Default
```
...
- Default (`512MB`) supports common large base layers (for example, `postgres`)
