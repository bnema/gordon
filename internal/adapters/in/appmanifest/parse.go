...
func toDomainService(raw rawService) (domain.AppService, error) {
	...
	for _, r := range raw.RCON {
		svc.RCON = append(svc.RCON, domain.AppRCONInterface{
			Entrypoint:   r.Entrypoint,
			Port:         r.Port,
			Publish:      r.Publish,
			Public:       r.Public,
			TrustedCIDRs: append([]string(nil), r.TrustedCIDRs...),
		})
	}
