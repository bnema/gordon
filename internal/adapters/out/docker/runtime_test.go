...
func TestRuntimeInspectNativeIDMappingsFailClosed(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			runtime, _ := newNativeInspectTestRuntime(t, containerID, cap.Known(), nil, test.status, test.body)
			inspected, err := runtime.InspectContainer(t.Context(), containerID)
			require.NoError(t, err)
...
func TestRuntimeInspectNativeProofDoesNotLeakResponseValues(t *testing.T) {
	...
	runtime, _ := newNativeInspectTestRuntime(t, containerID, cap.Known(), nil, http.StatusOK, body)
	var logs bytes.Buffer
...
func TestRuntimeInspectDoesNotExtendNativeAuthority(t *testing.T) {
	...
	runtime, requests := newNativeInspectTestRuntime(t, invalidID, cap.Known(), nil, http.StatusOK, validNativeIDMappings(21002))
