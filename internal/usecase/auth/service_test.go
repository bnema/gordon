...
func TestExtendTokenSkipsEphemeral(t *testing.T) {
	...
	// Generate an ephemeral access token (≤MaxAccessTokenLifetime)
...
func TestIsEphemeralAccessTokenRejectsFutureIat(t *testing.T) {
	...
	// Craft a token with iat = now+10min, exp = now+15min (age appears as -10min → negative → < MaxAccessTokenLifetime).
...
func TestValidateToken_IsEphemeral_EphemeralToken(t *testing.T) {
	// Ephemeral access tokens (generated by GenerateAccessToken) should have IsEphemeral = true
	svc := NewService(Config{
		Enabled:     true,
		AuthType:    domain.AuthTypeToken,
		TokenSecret: []byte("test-secret-key-for-jwt-signing"),
	...
	tokenStr, err := svc.GenerateAccessToken(ctx, "testuser", []string{"repository:myrepo:pull"}, 5*time.Minute)
	require.NoError(t, err)

	claims, err := svc.ValidateToken(ctx, tokenStr)
	require.NoError(t, err)
	assert.True(t, claims.IsEphemeral, "ephemeral access token should have IsEphemeral = true")
}

func TestValidateToken_IsEphemeral_StoredToken(t *testing.T) {
	// Long-lived stored tokens should have IsEphemeral = false
	tokenStore := mocks.NewMockTokenStore(t)
	...
	tokenStr, err := svc.GenerateToken(ctx, "testuser", []string{"push", "pull"}, MaxAccessTokenLifetime+time.Minute)
	require.NoError(t, err)

	tokenStore.EXPECT().
		GetToken(mock.Anything, "testuser").
		Return(tokenStr, capturedToken, nil)
	tokenStore.EXPECT().
		IsRevoked(mock.Anything, capturedToken.ID).
		Return(false, nil)
	...
	claims, err := svc.ValidateToken(ctx, tokenStr)
	require.NoError(t, err)
	assert.False(t, claims.IsEphemeral, "stored token should have IsEphemeral = false")
}

func TestValidateToken_IsEphemeral_NeverExpiringToken(t *testing.T) {
	// Never-expiring tokens must not be ephemeral
	tokenStore := mocks.NewMockTokenStore(t)
	...
	tokenStr, err := svc.GenerateToken(ctx, "ci-bot", []string{"push"}, 0)
	require.NoError(t, err)
	...
	claims, err := svc.ValidateToken(ctx, tokenStr)
	require.NoError(t, err)
	assert.False(t, claims.IsEphemeral, "never-expiring token should have IsEphemeral = false")
}
