...
func DockerCompatibilityPreflight(ctx context.Context) error {
	...
	cmd, err := newIsolatedCommand(ctx, "docker", []string{"info", "--format", "{{.ServerVersion}}"}, nil, nil, true)
	if err != nil {
		return fmt.Errorf("docker compatibility runtime unavailable: prepare docker info: %w", err)
	}
...
func dockerCompatibilityOutput(ctx context.Context, args ...string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("prepare docker command: %w", err)
	}
	output, err := cmd.Output()
	if err != nil {
