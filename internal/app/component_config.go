...
func componentRoleConfig(cfg Config, role domain.ComponentRole, migrationID, controlListenAddress, controlEndpoint string, controlRouting map[string]any, migrationProbeEnabled bool, publishedHostIP string) map[string]any {
	...
	case domain.ComponentRoleRuntime:
		return map[string]any{
			...
			"runtime": map[string]any{"listen_address": migrationRuntimeSocketEndpoint(migrationID), "token_env": "GORDON_COMPONENT_RUNTIME_TOKEN", "registry_storage_root": filepath.Join(resolveDataDir(cfg.Server.DataDir), "registry"), "migration_state_root": filepath.Join(resolveDataDir(cfg.Server.DataDir), "migration"), "managed_control_secrets_volume": managedControlSecretsVolumeName(resolveDataDir(cfg.Server.DataDir))},
